...
// Get API to fetch a single agency by agencyId
agency_router.get("/api/getAgency/:agencyId", async (req, res) => {
  try {
    const { agencyId } = req.params;
    const agency = await agency_model.find({ agencyId: agencyId }); // Fetch agency by ID
    ...
    res.status(500).json({
      Status: "Failed",
      msg: `Error fetching agency. Error: ${error.message}`,
    });
  }
});
...
agency_router.put("/api/update_plantOperator", async (req, res) => {
  ...
  try {
    ...
    if (!agencyData) {
      ...
      return res.status(200).json({
        Status: "Success",
        message: "Plant Operator updated successfully",
        updatedEntry,
      });

  } catch (error) {
    console.error("Error Updating Plant Operator:", error);
    res.status(500).json({
      Status: "Error",
...
agency_router.post("/api/agencyLogin", async (req, res) => {
  ...
  try {
    ...
    if (agencyData.passwordReset === 0 && password === "123") {
      res.status(200).json({
        Status: "Reset Password",
        msg: "Login successful as Agency.",
        user: agencyId
...
// Agency Login API
agency_router.post("/api/masterLogin", async (req, res) => {
  console.log("Request body login:", req.body);
  ...
  if (!masterId || !password) {
    return res.status(400).json({
      Status: "Failed",
      msg: "masterId and password are required",
    });
  }

  try {
    // Query the database to find the super admin by sAdminId
    const masterData = await superAdmin_model.findOne({ sAdminId: masterId });
    ...
    if (masterData.sAdminId === masterId && password === "123") {
      res.status(200).json({
        Status: "Success",
        msg: "Login successful as Master.",
        user: masterId
      });
    }
  } catch (error) {
...
agency_router.get("/api/agencyHome", verifyAgency, (req, res) => {
