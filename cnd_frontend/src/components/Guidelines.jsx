...
const Guidelines = ({
  ...
  const handleFileUpload = async () => {
    ...
    formData.append("category", "guideline");
  ...
  const fetchFiles = async () => {
    try {
      ...
      const data = await response.json();
      setDocuments(data.filter((d) => d.category === "guideline"));
    } catch (error) {
  ...
  //   }
  //   formData.append("category", 'guideline'); // Attach category info
  //   formData.append("agency", agency);
  ...
  return (
    ...
    //             )}
    //             <span
    //               className="inline-block cursor-pointer text-[#07426a] font-bold"
    //               onClick={handleBack}
    //             >
    //               ⬅️ Back
    //             </span>
