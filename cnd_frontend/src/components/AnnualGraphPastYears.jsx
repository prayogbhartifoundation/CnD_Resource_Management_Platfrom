...
const AnnualGraphPastYears = ({fyStart, fyStr }) => {
  ...
  useEffect(() => {
    ...
    const fyStartYear = fyStart ? parseInt(fyStart) : 2025;
    const fyString = fyStr || `${fyStartYear}-${fyStartYear + 1}`;

    const categories = deptOfftake.sort((a, b) => +a.seq - +b.seq).map((item) => item.department);

    // Compute actual and target values
    const actualValues = deptOfftake.sort((a, b) => +a.seq - +b.seq).map((dep) => {
    ...
    const targetValues = deptOfftake.sort((a, b) => +a.seq - +b.seq).map((dep) => {
      const targetObj = dep?.annualTarget?.find((t) => t.finYear === fyString);
      ...
      tooltip: {
        ...
        y: {
          formatter: (val, { dataPointIndex }) => {
            ...
            return `Actual: ${actual.toFixed(2)} MT | Target: ${target.toFixed(
              2
            )} MT`;
          },
        },
      ...
      annotations: {
        ...
        points: categories.map((dept, i) => ({
          ...
          label: {
            ...
            offsetY: -30, // adjust to appear just above % label
          },
        })),
      },
    });
...
export default AnnualGraphPastYears;
