...
const Publications = ({
  ...
  const categories = [
    { name: "Strategy by Promoting Processing of C&D waste by Niti Aayog" },
    { name: "Resource Efficiency Inittive." },
    { name: "Training Manual on C&D Waste in Indian Cities & Town" },
    { name: "Effective Recieving, Recycling, Reuse of C&D Waste by CPWD " },
  ];
  ...
  const fetchFiles = async () => {
    try {
      ...
      const data = await response.json();
      setDocuments(data.filter((d) => d.category === "publication"));
    } catch (error) {
  ...
  //   }
  //   formData.append("category", 'publication'); // Attach category info
  //   formData.append("agency", agency);
  ...
  return (
    ...
    //         className="border-none text-sm outline-none px-2 py-1 bg-white"
    //       />
    ...
    //         <h3 className="flex items-center text-[#393735] justify-between px-4">
    ...
    //               <li
    //                 key={index}
    //                 className="p-2 my-1 bg-orange-300 rounded cursor-pointer text-left flex justify-between border border-black"
    //               >
    //                 <div
    //                   onClick={(e) => {
    //                     e.preventDefault();
    ...
    <div className="w-[95%] mx-auto my-1 rounded-lg bg-white p-5 shadow-md">
      {/* Header */}
      {/* <div className="flex flex-wrap justify-between items-center bg-[#02AB6A] text-gray-800 px-5 py-2 rounded-t-lg gap-2">
    <h2 className="font-semibold text-lg sm:text-xl">Publications</h2>
    ...
    <div className="flex items-center bg-white px-2 py-1 rounded w-full sm:w-auto">
      <input
        ...
        value={searchTerm}
        onChange={(e) => setSearchTerm(e.target.value)}
        className="border-none text-sm outline-none px-2 py-1 bg-white w-full sm:w-auto"
      />
      <FontAwesomeIcon icon={faMagnifyingGlass} className="text-[#07426a] mr-2" />
    </div>
  </div> */}
      ...
      {user && (
        <button
          ...
          } text-white px-6 py-2 mt-4 rounded-lg font-medium hover:bg-[#027348] ${
        ...
        <div className="text-center">
          <div className="mt-4 overflow-x-auto">
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-orange-200 text-left">
                  ...
                  <th className="p-2 border text-center">Download</th>
                </tr>
              </thead>
                  ...
                  .map((doc, index) => {
                    ...
                    return (
                      <tr key={index} className="hover:bg-gray-100">
                        ...
                        <td className="p-2 border break-words">{fileName}</td>

                        <td className="p-2 border">{doc?.agency}</td>
            ...
            {documents.filter((doc) =>
              ...
              <p className="text-gray-600 italic mt-3 text-center">
                No documents found.
              </p>
            )}
          </div>
