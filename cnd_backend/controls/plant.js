...
plant_router.get("/api/getPlantsLocation", async (req, res) => {
  try{
    const plants = await plant_model.find(
      {
        "mapLocLatLong.lat": { $exists: true },
        "mapLocLatLong.lng": { $exists: true },
         location: { $ne: "Varanasi" }, 
        location: { $ne: "varanasi" }, 
      },
      {
        name: 1,
    ...
    res.status(500).json({
      Status: "Failed",
      msg: `Error fetching plants Location. Error: ${error.message}`,
    })
  }
})
