...
prod_router.post("/api/add_product", async (req, res) => {
  ...
  try {
    ...
    if (!prodName || !plantId) {
      return res
        .status(400)
        .json({ message: "Product name and Plant ID are required" });
    }
    ...
    if (product) {
      ...
      const plantIndex = product.plantWise.findIndex(
        (p) => p.plantId === plantId
      );

      if (plantIndex > -1) {
        // Update existing plant data
        product.plantWise[plantIndex] = {
          plantId,
          qnt,
          dsr,
          details,
      ...
      const lastProduct = await prod_model
        .findOne({ prodId: { $regex: /^PROD\d+$/ } })
        .sort({ prodId: -1 });

      let nextNumber = 1;

      if (lastProduct && lastProduct.prodId) {
        const lastNumber = parseInt(lastProduct.prodId.replace("PROD", ""), 10);
        nextNumber = lastNumber + 1;
      }

      let newProdId = `PROD${nextNumber}`;
      let exists = await prod_model.findOne({ prodId: newProdId });

      while (exists) {
        nextNumber++;
        newProdId = `PROD${nextNumber}`;
        exists = await prod_model.findOne({ prodId: newProdId });
      }
...
prod_router.post("/api/update_product", async (req, res) => {
  try {
    ...
    for (const { prodName, unit } of prodDetails) {
      const product = await prod_model.findOne({ prodName });
